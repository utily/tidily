...
class Handler implements Converter<string>, Formatter {
	...
	format(unformated: StateEditor): Readonly<State> & Settings {
		...
		return { ...result, type: "text", autocomplete: "cc-number", length: issuer.length.slice(1) as [number | undefined, number | undefined], pattern: issuer.verification, classes: ["issuer-" + issuer.icon] }
	}
...
const defaultIssuer: CardIssuer = { name: "unknown", verification: /^\d{19}$/, identification: /^\d/, length: [16, 16, 19], icon: "generic" }
